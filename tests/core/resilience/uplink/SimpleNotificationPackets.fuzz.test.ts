...
describe('SimpleNotificationPackets Resilience (Fuzzing)', () => {
    ...
    it('FEATURE REGRESSION: should throw a detailed BoksProtocolError for payloads that are too short', () => {
      fc.assert(
        fc.property(fc.uint8Array({ maxLength: 0 }), (payload) => {
          try {
            NotifyCodeGenerationProgressPacket.fromRaw(buildMockRawPacket(NotifyCodeGenerationProgressPacket.opcode, payload));
    ...
    it('FEATURE REGRESSION: should throw a detailed BoksProtocolError for payloads that are too short', () => {
      fc.assert(
        fc.property(fc.uint8Array({ minLength: 0, maxLength: 1 }), (payload) => {
          try {
            AnswerDoorStatusPacket.fromRaw(buildMockRawPacket(AnswerDoorStatusPacket.opcode, payload));
            expect.unreachable('Should have thrown an error');
          } catch (error: any) {
            expect(error.name).toBe('BoksProtocolError');
            expect(error.context).toBeDefined();
  ...
  describe('NotifyDoorStatusPacket', () => {
    ...
    it('FEATURE REGRESSION: should throw a detailed BoksProtocolError for payloads that are too short', () => {
      fc.assert(
        fc.property(fc.uint8Array({ minLength: 0, maxLength: 1 }), (payload) => {
          try {
            NotifyDoorStatusPacket.fromRaw(buildMockRawPacket(NotifyDoorStatusPacket.opcode, payload));
            expect.unreachable('Should have thrown an error');
          } catch (error: any) {
            expect(error.name).toBe('BoksProtocolError');
            expect(error.context).toBeDefined();
    ...
    it('FEATURE REGRESSION: should throw a detailed BoksProtocolError for payloads that are too short', () => {
      fc.assert(
        fc.property(fc.uint8Array({ minLength: 0, maxLength: 3 }), (payload) => {
          try {
            NotifyCodesCountPacket.fromRaw(buildMockRawPacket(NotifyCodesCountPacket.opcode, payload));
            expect.unreachable('Should have thrown an error');
          } catch (error: any) {
            expect(error.name).toBe('BoksProtocolError');
            expect(error.context).toBeDefined();
  ...
  describe('NotifyLogsCountPacket', () => {
    ...
    it('FEATURE REGRESSION: should throw a detailed BoksProtocolError for payloads that are too short', () => {
      fc.assert(
        fc.property(fc.uint8Array({ minLength: 0, maxLength: 1 }), (payload) => {
          try {
            NotifyLogsCountPacket.fromRaw(buildMockRawPacket(NotifyLogsCountPacket.opcode, payload));
            expect.unreachable('Should have thrown an error');
          } catch (error: any) {
            expect(error.name).toBe('BoksProtocolError');
            expect(error.context).toBeDefined();
  ...
  it('FEATURE REGRESSION: NotifyCodeGenerationErrorPacket should safely handle arbitrary payload lengths', () => {
    fc.assert(
      fc.property(fc.uint8Array({ minLength: 0, maxLength: 256 }), (payload) => {
        const packet = NotifyCodeGenerationErrorPacket.fromRaw(buildMockRawPacket(NotifyCodeGenerationErrorPacket.opcode, payload));
  ...
  it('FEATURE REGRESSION: NotifySetConfigurationSuccessPacket should safely handle arbitrary payload lengths', () => {
    fc.assert(
      fc.property(fc.uint8Array({ minLength: 0, maxLength: 256 }), (payload) => {
        const packet = NotifySetConfigurationSuccessPacket.fromRaw(buildMockRawPacket(NotifySetConfigurationSuccessPacket.opcode, payload));
  ...
  it('FEATURE REGRESSION: ValidOpenCodePacket should safely handle arbitrary payload lengths', () => {
    fc.assert(
      fc.property(fc.uint8Array({ minLength: 0, maxLength: 256 }), (payload) => {
        const packet = ValidOpenCodePacket.fromRaw(buildMockRawPacket(ValidOpenCodePacket.opcode, payload));
  ...
  describe('OperationErrorPacket', () => {
    ...
    it('FEATURE REGRESSION: should throw a detailed BoksProtocolError for payloads that are too short', () => {
      fc.assert(
        fc.property(fc.uint8Array({ maxLength: 0 }), (payload) => {
          try {
            OperationErrorPacket.fromRaw(buildMockRawPacket(OperationErrorPacket.opcode, payload));
            expect.unreachable('Should have thrown an error');
          } catch (error: any) {
            expect(error.name).toBe('BoksProtocolError');
            expect(error.context).toBeDefined();
