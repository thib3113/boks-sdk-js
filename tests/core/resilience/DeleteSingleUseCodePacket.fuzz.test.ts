...
describe('DeleteSingleUseCodePacket Resilience (Fuzzing)', () => {
  it('FEATURE REGRESSION: should securely reject invalid constructor arguments with BoksProtocolError', () => {
    ...
    fc.assert(
      fc.property(
        ...
        (configKey, pin) => {
          try {
            const packet = new DeleteSingleUseCodePacket({ configKey: configKey, pin: pin });
            ...
            expect(packet.opcode).toBe(0x02); // DeleteSingleUseCodePacket opcode
          } catch (e) {
