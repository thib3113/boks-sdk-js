...
describe('DownlinkPackets Resilience (Fuzzing)', () => {
  const downlinkPackets = [
    AskDoorStatusPacket,
    CountCodesPacket,
  ...
  downlinkPackets.forEach((PacketClass) => {
    describe(PacketClass.name, () => {
      it(`FEATURE REGRESSION: should safely handle arbitrary payload lengths without crashing`, () => {
        fc.assert(
          fc.property(fc.uint8Array({ minLength: 0, maxLength: 256 }), (payload) => {
            try {
              const packet = PacketClass.fromRaw(payload);
              expect(packet).toBeInstanceOf(PacketClass);
              expect(packet.opcode).toBe(PacketClass.opcode);
            } catch (e: any) {
