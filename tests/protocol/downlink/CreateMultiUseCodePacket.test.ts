...
describe('CreateMultiUseCodePacket', () => {
  ...
  it('should parse from payload correctly', () => {
    ...
    const packet = CreateMultiUseCodePacket.fromRaw(buildMockRawPacket(CreateMultiUseCodePacket.opcode, payload));
    expect(packet.configKey).toBe(validKey);
    expect(packet.pin).toBe(validPin);
  });
  ...
  it('should fail parsing if payload is too short', () => {
    ...
    expect(() => CreateMultiUseCodePacket.fromRaw(buildMockRawPacket(CreateMultiUseCodePacket.opcode, shortPayload))).toThrowError(
      BoksProtocolError
    );
  });
