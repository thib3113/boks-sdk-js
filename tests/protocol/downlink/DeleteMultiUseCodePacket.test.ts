...
describe('DeleteMultiUseCodePacket', () => {
  ...
  it('should construct with valid parameters', () => {
    const packet = new DeleteMultiUseCodePacket(validKey, validPin);
  ...
  it('should throw INVALID_CONFIG_KEY for invalid config key format', () => {
     expect(() => new DeleteMultiUseCodePacket('invalid', validPin)).toThrowError(BoksProtocolError);
     try {
       new DeleteMultiUseCodePacket('invalid', validPin);
     } catch (e) {
  ...
  it('should throw INVALID_PIN_FORMAT for invalid pin', () => {
      expect(() => new DeleteMultiUseCodePacket(validKey, '123')).toThrowError(BoksProtocolError);
      expect(() => new DeleteMultiUseCodePacket(validKey, '12345C')).toThrowError(BoksProtocolError);

      try {
        new DeleteMultiUseCodePacket(validKey, '12345C');
      } catch (e) {
