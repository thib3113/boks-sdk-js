...
describe('PowerOffHistoryPacket', () => {
  ...
  it('should throw error on missing reason', () => {
    ...
    expect(() => PowerOffHistoryPacket.fromRaw(buildMockRawPacket(PowerOffHistoryPacket.opcode, payload))).toThrowError();
  });

  it('should output only mapped payload properties and opcode via toJSON', () => {
    const packet = PowerOffHistoryPacket.fromRaw(buildMockRawPacket(PowerOffHistoryPacket.opcode, new Uint8Array([0x00, 0x00, 0x3c, 0x01])));
