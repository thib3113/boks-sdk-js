...
describe('ScaleMeasureHistoryPacket', () => {
  ...
  describe('ScaleMeasureHistoryPacket very short payload edge case', () => {
    it('should throw BoksProtocolError if payload is less than 3 bytes', () => {
      ...
      expect(() => ScaleMeasureHistoryPacket.fromRaw(payload)).toThrow(BoksProtocolError);
    });
  });

  it('should output only mapped payload properties and opcode via toJSON', () => {
    const packet = ScaleMeasureHistoryPacket.fromRaw(new Uint8Array([0x00, 0x00, 0x0a, 0x01, 0x02, 0x03, 0x04]));
  ...
  it('should retain the exact raw payload when constructed from hex via factory', () => {
    const dummyPayload = new Uint8Array([ScaleMeasureHistoryPacket.opcode, 0x05, 0x01, 0x02, 0x03, 0x04, 0x05, 0x00]);
    try {
      const packet = ScaleMeasureHistoryPacket.fromRaw(dummyPayload, { strict: false });
      if (packet) {
