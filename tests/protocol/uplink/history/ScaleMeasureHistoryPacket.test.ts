...
describe('ScaleMeasureHistoryPacket', () => {
  ...
  describe('ScaleMeasureHistoryPacket very short payload edge case', () => {
    it('should throw BoksProtocolError if payload is less than 3 bytes', () => {
      ...
      expect(() => ScaleMeasureHistoryPacket.fromRaw(buildMockRawPacket(ScaleMeasureHistoryPacket.opcode, payload))).toThrow(BoksProtocolError);
    });
  });

  it('should output only mapped payload properties and opcode via toJSON', () => {
    const packet = ScaleMeasureHistoryPacket.fromRaw(buildMockRawPacket(ScaleMeasureHistoryPacket.opcode, new Uint8Array([0x00, 0x00, 0x0a, 0x01, 0x02, 0x03, 0x04])));
