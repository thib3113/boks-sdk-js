...
describe('CodeBleValidHistoryPacket', () => {
  ...
  it('should output only mapped payload properties and opcode via toJSON', () => {
    const packet = CodeBleValidHistoryPacket.fromRaw(new Uint8Array([0, 0, 10, 49, 50, 51, 52, 53, 54, 0, 0, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11]));
  ...
  it('should retain the exact raw payload when constructed from hex via factory', () => {
    const dummyPayload = new Uint8Array([CodeBleValidHistoryPacket.opcode, 0x05, 0x01, 0x02, 0x03, 0x04, 0x05, 0x00]);
    try {
      const packet = CodeBleValidHistoryPacket.fromRaw(dummyPayload, { strict: false });
      if (packet) {
