...
describe('ScaleTareLoadedPacket', () => {
  ...
  it('should parse from payload correctly', () => {
    const data = new Uint8Array([0x01, 0x02]);
    const packet = ScaleTareLoadedPacket.fromRaw(buildMockRawPacket(ScaleTareLoadedPacket.opcode, data));
    expect(packet.data).toEqual(data);
  });

  it('should output only mapped payload properties and opcode via toJSON', () => {
    const packet = ScaleTareLoadedPacket.fromRaw(buildMockRawPacket(ScaleTareLoadedPacket.opcode, new Uint8Array([0x01, 0x02])));
