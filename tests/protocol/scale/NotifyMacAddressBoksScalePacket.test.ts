...
describe('NotifyMacAddressBoksScalePacket', () => {
  it('should parse correctly', () => {
    ...
    const packet = NotifyMacAddressBoksScalePacket.fromRaw(buildMockRawPacket(NotifyMacAddressBoksScalePacket.opcode, payload));
  ...
  it('should handle empty payload', () => {
    ...
    expect(() => NotifyMacAddressBoksScalePacket.fromRaw(buildMockRawPacket(NotifyMacAddressBoksScalePacket.opcode, payload))).toThrowError(
      BoksProtocolError
    );
  });

  it('should output only mapped payload properties and opcode via toJSON', () => {
    const packet = NotifyMacAddressBoksScalePacket.fromRaw(buildMockRawPacket(NotifyMacAddressBoksScalePacket.opcode, new Uint8Array([0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff])));
